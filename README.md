# Go Smart Solutions — Basic, Back-end, Database และ Edge Gateway
...
ตอนนี้เตรียม **Phase 1 — Go Basic ครบ 28 EP** แล้ว เรียนทีละ EP ตามจังหวะของตนเองได้ Phase 2–5 ยังเป็นแผน การมีไฟล์ครบไม่ใช่ข้อกำหนดให้รีบเรียน
...
```text
C:\Users\kopes\Documents\Codex\2026-09-17\go-youtube-go-iot-edge-gateway\Go
```

โฟลเดอร์ที่ผู้เรียนแนบ `C:\Users\kopes\Documents\L\Go` มีอยู่แต่ไม่มีไฟล์ ณ เวลาตรวจ จึงไม่ได้แก้ไขที่นั่น การเลือกตำแหน่งนี้อิงคำขอให้สร้างใน workspace ปัจจุบัน
...
## เหตุผลที่ใช้หนึ่ง module ต่อ EP
...
**แนวทางออกแบบหลักสูตร:** EP1 ใช้ module Hello Go เดิม ส่วน EP2–28 มี module แยกแต่ละ EP บทเรียนที่ทำพังระหว่างฝึกจะไม่ขัดขวางการทดสอบบทอื่น และบท MQTT ในอนาคตจะไม่เพิ่มไลบรารีให้บทแรก รันคำสั่งจากโฟลเดอร์บทที่กำลังเรียน ไม่รัน `go test ./...` จากราก `Go` ซึ่งไม่มี module

บทแรกใช้ชื่อ `example.com/go-course/hello` เป็นชื่อสำหรับแบบฝึกหัด ไม่มีการเผยแพร่หรือเชื่อมต่อโดเมนนี้ ใช้เฉพาะ `fmt` ที่ติดมากับ Go ไม่มีไลบรารีภายนอก ไม่ต้อง `go get` หรือ `go mod init` ซ้ำ เพราะเตรียม `go.mod` แล้ว
...
**ทางเลือก:** module เดียวทั้งหลักสูตรก็ทำได้ แต่มีโอกาสที่แบบฝึกหัดหนึ่งจะทำให้คำสั่งตรวจทั้งหลักสูตรล้มเหลว ยังไม่ใช้ `go.work`, framework หรือ Docker เพื่อไม่เพิ่มเรื่องที่ต้องเรียนในตอนแรก
...
`lessons/` เก็บตัวอย่าง starter และเฉลยที่ตั้งใจแจก ส่วนการแก้โค้ดของผู้เรียนใช้ `practics/` ซึ่ง `.gitignore` ไม่เพิ่มเข้า Git ตามปกติ เช่น `practics/phase-01/ep01-hello-go` มี module แยก ทำให้ฝึกผิดได้โดยไม่ทำให้การตรวจตัวอย่างต้นฉบับเสีย อ่าน [วิธีฝึกและสร้างพื้นที่หลัง clone](docs/PRACTICE.md)
...
เริ่มใช้ Git repository ภายในโฟลเดอร์ Go เมื่อ 2026-09-20 บน branch `main` เพื่อเก็บประวัติหลักสูตร ยังไม่ได้ตั้ง remote หรือ push ไปบริการภายนอก `.gitignore` แยกงานฝึกออกจากไฟล์ที่เพิ่มตามปกติ การ ignore ไม่ลบไฟล์ที่เคยติดตามแล้วและไม่ป้องกันการฝืนเพิ่มด้วย `git add -f` จึงไม่ใช่ที่เก็บข้อมูลลับ
