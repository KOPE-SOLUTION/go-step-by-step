...
func TestBuildReportMissingID(t *testing.T) {
	lines, accepted := buildReport([]Reading{
		{DeviceID: "", Celsius: 27},
		{DeviceID: "sensor-02", Celsius: 28},
	}, 30)
	if accepted != 1 || len(lines) != 2 {
		t.Fatalf("lines=%v accepted=%d; want 2 lines and 1 accepted", lines, accepted)
	}
	if lines[0] != ": ERROR: device ID is required" || lines[1] != "sensor-02: 28.0 C [OK]" {
		t.Errorf("unexpected report: %v", lines)
	}
}
